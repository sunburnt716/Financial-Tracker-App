...
// -------------------- SETUP & AUTH --------------------
// (Keep this secure in production!)
...
if (!fs.existsSync(keyFilePath)) {
  ...
  try {
    fs.writeFileSync(keyFilePath, process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON, { encoding: "utf8" });
  } catch (err) {
    console.error("Error writing temp key file:", err);
  }
}
...
export function parseReceiptData(doc) {
  ...
  const itemNames = entities
    ...
    .sort((a, b) => parseInt(a.textAnchor.textSegments[0].startIndex) - parseInt(b.textAnchor.textSegments[0].startIndex))
    .map((e) => e.mentionText);
...
export function parseBrokerageStatement(doc) {
  ...
  entities.forEach(entity => {
    ...
    if (type === 'beginning_portfolio_value') result.beginning_balance = parseMoney(value);
    if (type === 'ending_portfolio_value') result.ending_balance = parseMoney(value);
    ...
    if (type === 'holding_row' && entity.properties) {
      ...
      entity.properties.forEach(child => {
        ...
        if (child.type === 'num_of_shares') holding.shares = parseNumber(child.mentionText);
        if (child.type === 'market_price') holding.price = parseMoney(child.mentionText);
        if (child.type === 'market_value') holding.total_value = parseMoney(child.mentionText);
      });
  ...
  entities.forEach(entity => {
    ...
    if (type === 'holding_row' && entity.properties) {
      ...
      entity.properties.forEach(child => {
        ...
        if (child.type === 'market_value') holding.market_value = parseMoney(val);
        if (child.type === 'price_per_share') holding.price_per_share = parseMoney(val);
        if (child.type === 'cost_basis') holding.cost_basis = parseMoney(val);
        if (child.type === 'stock_dividend') holding.stock_dividend = parseMoney(val);
        if (child.type === 'stock_purchase_date') holding.purchase_date = new Date(val);
      });
